...
SimG4SingleParticleGeneratorToolPt::SimG4SingleParticleGeneratorToolPt(const std::string& type,
  ...
  declareProperty("ptMin", m_energyMin = 1 * CLHEP::GeV, "Minimum energy of generated particles");
  declareProperty("ptMax", m_energyMax = 1 * CLHEP::TeV, "Maximum energy of generated particles");
...
G4Event* SimG4SingleParticleGeneratorToolPt::g4Event() {
  ...
  double pzsq = particleEnergy*particleEnergy * tanhetasq / (1. - tanhetasq);

  G4PrimaryVertex* vertex = new G4PrimaryVertex(particlePosition, 0.);
  G4PrimaryParticle* part = new G4PrimaryParticle(particleDef);

  part->SetMass(mass);
  //part->SetKineticEnergy(std::sqrt(particleEnergy*particleEnergy + pzsq));
  part->SetMomentum(particleEnergy*std::cos(phi), particleEnergy*std::sin(phi), std::sqrt(pzsq));
...
StatusCode SimG4SingleParticleGeneratorToolPt::saveToEdm(const G4PrimaryVertex* aVertex,
